 ...
 */
export class PrincipalMemoryGuard implements MemoryGuard {
  async evaluateWrite(event: CognitiveEvent): Promise<GuardDecision> {
    const denied =
      event.trustClass === "blocked" ||
  ...
  async evaluateRecall(event: CognitiveEvent): Promise<RecallDecision> {
