...
import type { MemoryItem } from "../memory/types/dual-mode-memory.types";
...
test("recall foundation prefers high-quality adopted memory and excludes suppressed/rejected", () => {
...
test("recall foundation keeps downgraded/warning items but ranks trusted higher deterministically", () => {
  const service = new CognitiveMemoryService();
...
test("recall ranking uses reinforcement score as bounded factor", () => {
  const service = new CognitiveMemoryService();
...
test("recall foundation excludes malformed and anomaly-risk candidates safely", () => {
  const service = new CognitiveMemoryService();
