...
test("dispatch guard deny blocks adaptor dispatch before invocation", async () => {
  class CountingLanguageCapability extends LanguageCapability {
    public calls = 0;
    override async invoke(request: CapabilityRequestEnvelope): Promise<CapabilityResultEnvelope> {
      this.calls += 1;
  ...
  const orchestrator = new CapabilityMeshOrchestrator(new CapabilityRegistry(), [counting], undefined, new DispatchGuard());
  const outcome = await orchestrator.invoke({
...
test("dispatch guard safe mode redirect blocks adaptor dispatch", async () => {
  class CountingLanguageCapability extends LanguageCapability {
    public calls = 0;
    override async invoke(request: CapabilityRequestEnvelope): Promise<CapabilityResultEnvelope> {
      this.calls += 1;
...
test("orchestrator classifies not-found capability deterministically", async () => {
...
test("orchestrator classifies invalid request envelope deterministically", async () => {
  const orchestrator = new CapabilityMeshOrchestrator(new CapabilityRegistry(), [new LanguageCapability()]);
  const outcome = await orchestrator.invoke({
    requestId: " ",
    sessionId: "",
...
test("orchestrator classifies adaptor/provider execution failure", async () => {
  class ThrowingCapability implements CapabilityAdaptor {
    getCapabilityDefinition(): CapabilityDefinition {
      return {
  ...
  const orchestrator = new CapabilityMeshOrchestrator(new CapabilityRegistry(), [new ThrowingCapability()]);
  const outcome = await orchestrator.invoke({
...
test("orchestrator returns normalized classification on success results", async () => {
  const orchestrator = new CapabilityMeshOrchestrator(new CapabilityRegistry(), [new LanguageCapability()]);
  const outcome = await orchestrator.invoke({
...
test("dispatch guard reroutes dispatch to fallback adaptor target", async () => {
  class CountingLanguageCapability extends LanguageCapability {
    public calls = 0;
    override async invoke(request: CapabilityRequestEnvelope): Promise<CapabilityResultEnvelope> {
      this.calls += 1;
  ...
  class CountingRetrievalCapability extends RetrievalCapability {
    public calls = 0;
    override async invoke(request: CapabilityRequestEnvelope): Promise<CapabilityResultEnvelope> {
      this.calls += 1;
      return super.invoke(request);
  ...
  const orchestrator = new CapabilityMeshOrchestrator(
    new CapabilityRegistry(),
    [language, retrieval, new VerificationCapability()],
    undefined,
    new DispatchGuard()
  );
  const outcome = await orchestrator.invoke({
    ...
    purpose: "normalize",
    input: "hello",
    trace: { dispatchGuardRerouteCapabilityId: CAPABILITY_IDS.RETRIEVAL },
    createdAt: new Date().toISOString(),
  });
...
test("dispatch guard degraded and audit outcomes mark deterministic warnings", async () => {
  const orchestrator = new CapabilityMeshOrchestrator(new CapabilityRegistry(), [
    new LanguageCapability(),
    new VerificationCapability(),
  ]);
...
test("dispatch guard require audit marks warning and allows deterministic dispatch", async () => {
  const orchestrator = new CapabilityMeshOrchestrator(new CapabilityRegistry(), [new LanguageCapability()]);
  const outcome = await orchestrator.invoke({
...
test("execution ledger captures runtime and dispatch guard outcomes in capability flow", async () => {
  const orchestrator = new CapabilityMeshOrchestrator(new CapabilityRegistry(), [
    new LanguageCapability(),
    new VerificationCapability(),
  ]);
