...
test("canonicalizer maps equivalent runtime events to deterministic canonical semantics", () => {
  const timestamp = "2026-03-08T00:00:00.000Z";
  const meshEvent = canonicalizeExecutionLedgerEntry(
    {
      entryId: "exec_1",
      timestamp,
      category: "runtime",
  ...
  const orchestratorEvent = canonicalizeExecutionLedgerEntry(
    {
      entryId: "exec_2",
      timestamp,
      category: "runtime",
...
test("execution ledger emits canonical stage progression with deterministic sequence per execution", () => {
  ...
  assert.deepEqual(
    timeline.map((event) => event.stage),
    ["dispatch_evaluated", "runtime_evaluated", "execution_started", "execution_completed"]
  );
  assert.deepEqual(
    timeline.map((event) => event.sequenceNo),
    [1, 2, 3, 4]
  );
});

test("deny/reroute/degraded/safe-mode/failure outcomes are canonicalized predictably", () => {
  const ledger = new ExecutionLedger("");
  ledger.append({
    category: "dispatch",
...
test("stable identity and event id strategy is deterministic for same canonical input", () => {
  const input = {
    entryId: "exec_9",
  ...
  const first = canonicalizeExecutionLedgerEntry(input, { sequenceNo: 3, prevEventHash: "abc" });
  const second = canonicalizeExecutionLedgerEntry(input, { sequenceNo: 3, prevEventHash: "abc" });
...
test("active runtime and orchestrator paths expose canonical timeline snapshots", async () => {
