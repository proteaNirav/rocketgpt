...
import type { CanonicalTimelineEvent, ExecutionLedgerLikeEntry } from "./timeline-canonicalizer";
import { canonicalizeExecutionLedgerEntry } from "./timeline-canonicalizer";

export type LedgerIntegrityVerificationStatus = "valid" | "invalid" | "warning" | "partial";
export type LedgerIntegrityFindingSeverity = "error" | "warning";
export type LedgerIntegrityFindingScope = "record" | "stream" | "dataset";

export type LedgerIntegrityReasonCode =
  | "STRUCTURE_MISSING_REQUIRED_FIELD"
  | "STRUCTURE_INVALID_FIELD_VALUE"
  ...
  | "TIMESTAMP_INVALID"
  | "STAGE_TRANSITION_ANOMALY"
  | "STREAM_CONTINUES_AFTER_TERMINAL"
  | "STREAM_TERMINAL_DUPLICATE"
  | "LEDGER_TIMELINE_LENGTH_MISMATCH"
  | "LEDGER_TIMELINE_MISSING_EVENT"
  | "LEDGER_TIMELINE_ORPHAN_EVENT"
  | "LEDGER_TIMELINE_EVENT_MISMATCH"
  | "JSONL_PARSE_ERROR"
  | "PARTIAL_VERIFICATION_TIMELINE_DERIVED";

export interface LedgerIntegrityFinding {
  code: LedgerIntegrityReasonCode;
  severity: LedgerIntegrityFindingSeverity;
  scope: LedgerIntegrityFindingScope;
  message: string;
  executionId?: string;
  eventId?: string;
  sequenceNo?: number;
...
export interface LedgerIntegritySummary {
  status: LedgerIntegrityVerificationStatus;
  streamCount: number;
  recordCount: number;
...
export interface LedgerIntegrityVerificationResult {
...
export interface LedgerIntegrityInput {
  timelineEvents?: ReadonlyArray<CanonicalTimelineEvent>;
  ledgerEntries?: ReadonlyArray<ExecutionLedgerEntry>;
}
...
const TERMINAL_STAGES = new Set<CanonicalTimelineEvent["stage"]>([
  "execution_completed",
  "execution_failed",
  "execution_denied",
...
function addFinding(state: MutableState, finding: LedgerIntegrityFinding): void {
  state.findings.push(finding);
}

function invertCanonicalEventType(eventType: CanonicalTimelineEvent["eventType"]): ExecutionLedgerLikeEntry["eventType"] {
  switch (eventType) {
...
function asReasonObjects(codes?: string[]): Array<{ code: string; detail: string }> {
  return (codes ?? []).map((code) => ({ code, detail: "" }));
}

function toLedgerLikeFromTimeline(event: CanonicalTimelineEvent): ExecutionLedgerLikeEntry {
  return {
...
function verifyRequiredFields(event: CanonicalTimelineEvent, index: number, state: MutableState): void {
  const requiredFields: Array<keyof CanonicalTimelineEvent> = [
    "schemaVersion",
    "executionId",
    "eventId",
    "stableIdentity",
...
function verifyTimelineStream(
  executionId: string,
  events: Array<{ event: CanonicalTimelineEvent; index: number }>,
  state: MutableState
): { records: number; sequenceMin: number | null; sequenceMax: number | null; terminalStages: string[] } {
  ...
  const stableIdentitySeen = new Map<string, string>();
  let previous: CanonicalTimelineEvent | null = null;
  ...
  for (let i = 0; i < bySequence.length; i += 1) {
    ...
    const expected = canonicalizeExecutionLedgerEntry(toLedgerLikeFromTimeline(current), {
...
function buildSummary(state: MutableState, streamCount: number, recordCount: number): LedgerIntegritySummary {
  ...
  const invalidRecordIds = new Set(
    state.findings
      ...
      .map((finding) => finding.eventId as string)
  );
  const invalidRecordCount = invalidRecordIds.size;
  const validRecordCount = Math.max(0, recordCount - invalidRecordCount);
  const status: LedgerIntegrityVerificationStatus =
    errorCount > 0 ? "invalid" : state.partial ? "partial" : warningCount > 0 ? "warning" : "valid";
  return {
    status,
    streamCount,
    recordCount,
...
function verifyTimelineOnly(timelineEvents: ReadonlyArray<CanonicalTimelineEvent>, state: MutableState): {
  streamStats: LedgerIntegrityVerificationResult["streamStats"];
} {
  ...
  const streamMap = new Map<string, Array<{ event: CanonicalTimelineEvent; index: number }>>();
  for (let i = 0; i < timelineEvents.length; i += 1) {
    const event = timelineEvents[i]!;
  ...
  const streamStats: LedgerIntegrityVerificationResult["streamStats"] = [];
...
function deriveTimelineFromLedger(entries: ReadonlyArray<ExecutionLedgerEntry>): CanonicalTimelineEvent[] {
  const sequenceByExecutionId = new Map<string, number>();
  const prevHashByExecutionId = new Map<string, string>();
  const out: CanonicalTimelineEvent[] = [];
  for (const entry of entries) {
    ...
    const canonical = canonicalizeExecutionLedgerEntry(entry, { sequenceNo: nextSequence, prevEventHash: prev });
...
function compareLedgerDerivedAndTimeline(
  expected: ReadonlyArray<CanonicalTimelineEvent>,
  actual: ReadonlyArray<CanonicalTimelineEvent>,
  state: MutableState
): void {
  ...
  const actualByEventId = new Map(actual.map((event) => [event.eventId, event] as const));
  const expectedByEventId = new Map(expected.map((event) => [event.eventId, event] as const));
  ...
  for (const expectedEvent of expected) {
    ...
    if (actualEvent.stableIdentity !== expectedEvent.stableIdentity) mismatchedFields.push("stableIdentity");
    if (actualEvent.integrity.eventHash !== expectedEvent.integrity.eventHash) mismatchedFields.push("integrity.eventHash");
    if (actualEvent.integrity.prevEventHash !== expectedEvent.integrity.prevEventHash) mismatchedFields.push("integrity.prevEventHash");
    if (actualEvent.sequenceNo !== expectedEvent.sequenceNo) mismatchedFields.push("sequenceNo");
    if (actualEvent.executionId !== expectedEvent.executionId) mismatchedFields.push("executionId");
...
function normalizeFindings(findings: LedgerIntegrityFinding[]): LedgerIntegrityFinding[] {
...
export function verifyLedgerIntegrity(input: LedgerIntegrityInput): LedgerIntegrityVerificationResult {
  const state: MutableState = { findings: [], partial: false };
...
export function formatLedgerIntegritySummary(result: LedgerIntegrityVerificationResult): string {
  return [
...
export async function readJsonlRecords(filePath: string): Promise<{
  records: unknown[];
  parseFindings: LedgerIntegrityFinding[];
}> {
  try {
    const text = await readFile(filePath, "utf8");
    const lines = text.split(/\r?\n/).filter((line) => line.trim().length > 0);
    const records: unknown[] = [];
    const parseFindings: LedgerIntegrityFinding[] = [];
...
export async function verifyCanonicalTimelineJsonlFile(filePath: string): Promise<LedgerIntegrityVerificationResult> {
  const { records, parseFindings } = await readJsonlRecords(filePath);
  const typed = records.filter((record): record is CanonicalTimelineEvent => Boolean(record && typeof record === "object"));
  ...
  return {
    ...
    summary: {
      ...
      invalidRecordCount: new Set(
        findings
          .filter((finding) => finding.scope === "record" && finding.severity === "error" && finding.eventId)
          .map((finding) => finding.eventId as string)
      ).size,
      validRecordCount: Math.max(0, result.summary.recordCount - new Set(
        findings
          .filter((finding) => finding.scope === "record" && finding.severity === "error" && finding.eventId)
          .map((finding) => finding.eventId as string)
      ).size),
