import { readJsonlRecords, type LedgerIntegrityVerificationResult, verifyLedgerIntegrity } from "./ledger-integrity-verifier";
import { canonicalizeExecutionLedgerEntry, resolveCanonicalExecutionId, type CanonicalTimelineEvent } from "./timeline-canonicalizer";

export type SideEffectDriftStatus = "no_drift" | "drift_detected" | "warning" | "partial" | "inconclusive";
export type SideEffectDriftSeverity = "critical" | "high" | "medium" | "low";
export type SideEffectDriftScope = "side_effect" | "stream" | "dataset";

export type SideEffectDriftReasonCode =
...
export interface SideEffectDriftFinding {
  code: SideEffectDriftReasonCode;
  severity: SideEffectDriftSeverity;
  scope: SideEffectDriftScope;
  message: string;
  executionId?: string;
  eventId?: string;
  sequenceNo?: number;
...
export interface SideEffectDriftSummary {
  status: SideEffectDriftStatus;
  streamCount: number;
  recordCount: number;
  sideEffectIntentCount: number;
  sideEffectCompletionCount: number;
  matchedSideEffectCount: number;
  unmatchedIntentCount: number;
  unmatchedCompletionCount: number;
  driftFindingCount: number;
  warningCount: number;
  partial: boolean;
  integrityStatus?: LedgerIntegrityVerificationResult["summary"]["status"] | "not_validated";
}

export interface SideEffectDriftResult {
  summary: SideEffectDriftSummary;
  findings: SideEffectDriftFinding[];
  streamStats: Array<{
    executionId: string;
    intents: number;
    completions: number;
    ...
    completedAfterDenied: number;
    completedAfterTerminal: number;
  }>;
  integrity?: LedgerIntegrityVerificationResult;
}

export interface SideEffectDriftInput {
  ...
  integrityResult?: LedgerIntegrityVerificationResult;
  verifyIntegrityIfMissing?: boolean;
}
...
function addFinding(state: MutableState, finding: SideEffectDriftFinding): void {
  state.findings.push(finding);
}

function sideEffectSignature(event: CanonicalTimelineEvent): string {
  return `${event.action}|${event.source}|${event.target}`;
}

function deriveTimelineFromLedger(input: SideEffectDriftInput["ledgerEntries"]): CanonicalTimelineEvent[] {
  if (!input) {
    return [];
  }
  ...
  for (const entry of input) {
    ...
    const timelineEvent = canonicalizeExecutionLedgerEntry(entry, { sequenceNo, prevEventHash });
...
function normalizeFindings(findings: SideEffectDriftFinding[]): SideEffectDriftFinding[] {
...
function findIntegrityStatus(
  input: SideEffectDriftInput,
  timeline: ReadonlyArray<CanonicalTimelineEvent>
): LedgerIntegrityVerificationResult | undefined {
  if (input.integrityResult) {
    return input.integrityResult;
  }
  if (input.verifyIntegrityIfMissing) {
    return verifyLedgerIntegrity({ timelineEvents: timeline });
  }
  return undefined;
}

function detectStreamDrift(
  executionId: string,
  stream: Array<{ event: CanonicalTimelineEvent; index: number }>,
  state: MutableState
): SideEffectDriftResult["streamStats"][number] {
  ...
  const completionSignatureCounts = new Map<string, number>();
  const intentSignatureCounts = new Map<string, number>();
  let sequenceDispatchStarted: number | undefined;
  let sequenceExecutionStarted: number | undefined;
  let sequenceExecutionCompleted: number | undefined;
  let sequenceDenied: number | undefined;
  let sequenceSafeModeRedirect: number | undefined;
  let sequenceTerminal: number | undefined;
  ...
  for (const completion of completions) {
    ...
    let matchedIntent = intents.find(
      (intent) => !intent.matched && sideEffectSignature(intent.event) === completionSig && intent.event.sequenceNo <= completion.event.sequenceNo
    );

    if (!matchedIntent) {
      const sameTarget = intents.find(
        (intent) => !intent.matched && intent.event.target === completion.event.target && intent.event.sequenceNo <= completion.event.sequenceNo
      );
      const sameAction = intents.find(
        (intent) => !intent.matched && intent.event.action === completion.event.action && intent.event.sequenceNo <= completion.event.sequenceNo
      );
      const laterIntent = intents.find(
        (intent) => !intent.matched && sideEffectSignature(intent.event) === completionSig && intent.event.sequenceNo > completion.event.sequenceNo
      );
      if (laterIntent) {
        addFinding(state, {
    ...
    if (
      (sequenceDispatchStarted != null && completion.event.sequenceNo < sequenceDispatchStarted) ||
  ...
  if (sequenceExecutionCompleted != null) {
    const missingBeforeCompleted = intents.filter(
      (intent) => !intent.matched && intent.event.sequenceNo <= sequenceExecutionCompleted
    );
...
function buildSummary(
  timelineEvents: ReadonlyArray<CanonicalTimelineEvent>,
  streamStats: SideEffectDriftResult["streamStats"],
  findings: SideEffectDriftFinding[],
  partial: boolean,
  integrity?: LedgerIntegrityVerificationResult
): SideEffectDriftSummary {
  const driftFindingCount = findings.filter((finding) =>
    finding.code !== "INTEGRITY_NOT_VALIDATED" && finding.code !== "INTEGRITY_INVALID_INPUT" && finding.code !== "JSONL_PARSE_ERROR"
  ).length;
  ...
  let status: SideEffectDriftStatus = "no_drift";
...
export function detectSideEffectDrift(input: SideEffectDriftInput): SideEffectDriftResult {
  ...
  const streamStats: SideEffectDriftResult["streamStats"] = [];
...
export function formatSideEffectDriftSummary(result: SideEffectDriftResult): string {
  return [
...
export async function detectSideEffectDriftForTimelineJsonlFile(
  filePath: string,
  options?: { verifyIntegrity?: boolean }
): Promise<SideEffectDriftResult> {
  const { records, parseFindings } = await readJsonlRecords(filePath);
  const timeline = records.filter((record): record is CanonicalTimelineEvent => Boolean(record && typeof record === "object"));
  const integrity = options?.verifyIntegrity === true ? verifyLedgerIntegrity({ timelineEvents: timeline }) : undefined;
  const result = detectSideEffectDrift({
    timelineEvents: timeline,
    integrityResult: integrity,
    verifyIntegrityIfMissing: false,
  });
  const parseDriftFindings: SideEffectDriftFinding[] = parseFindings.map((finding) => ({
