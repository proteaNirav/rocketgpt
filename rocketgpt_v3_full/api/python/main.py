...
async def rate_limit(request, call_next):
    ...
    if now > reset: count, reset = 0, now+60
    if count >= MAX_REQ_PER_MIN: return PlainTextResponse("Too Many Requests", status_code=429)
...
def load_toolbase():
    with open(TOOLBASE_PATH,"r",encoding="utf-8") as f: return json.load(f)
...
def score(tool, kws, prefs):
    ...
    return sum([w["relevance"]*relevance, w["accessibility"]*accessibility, w["latency"]*latency, w["cost"]*cost, w["familiarity"]*familiarity])
...
def orchestrate(req: OrchestrateRequest):
    ...
    tools_out = [{
        "name": t["name"], "purpose": t.get("role"), "pricing": t.get("pricing"),
        "access_url": t.get("access_url"), "workflow_role": "primary" if i==0 else "supporting",
    ...
    ] if len(tools_out)==3 else ["Review ToolBase.json; not enough tools found."]
    ...
    return OrchestrateResponse(understood=understood, tools=tools_out, workflow=workflow, summary=summary)
...
async def groq_generate(prompt: str, model: str):
    if not (ENABLE_GROQ and GROQ_KEY): raise HTTPException(500, "Groq not enabled or key missing")
    payload = {"model": model, "messages":[{"role":"user","content": prompt}], "max_tokens": GEN_MAXTOK, "temperature":0.4, "stream": False}
    async with httpx.AsyncClient(timeout=GEN_TIMEOUT) as cx:
        r = await cx.post("https://api.groq.com/openai/v1/chat/completions",
                          headers={"Authorization": f"Bearer {GROQ_KEY}","Content-Type":"application/json"}, json=payload)
    if r.status_code >= 400: raise HTTPException(502, f"Groq upstream error {r.status_code}: {r.text[:180]}")
    data = r.json()
    return {"provider":"groq","model": data.get("model","llama3-8b-8192"), "output": data["choices"][0]["message"]["content"]}

from fastapi import Body
@app.post("/generate")
async def generate(req: GenerateRequest = Body(...)):
    if not ENABLE_AUTOPILOT: return {"error":"autopilot disabled"}
    quality = decide_quality(req.mode, req.persona)
    model = "llama3-8b-8192" if quality in ("speed","balanced") else "llama3-70b-8192"
...
async def plan_and_generate(req: PlanAndGenerateRequest = Body(...)):
    if not ENABLE_AUTOPILOT: return {"error":"autopilot disabled"}
    plan = orchestrate(OrchestrateRequest(task=req.task, persona=req.persona, mode=req.mode, org_prefs=req.org_prefs))
    quality = decide_quality(req.mode, req.persona)
    model = "llama3-8b-8192" if quality in ("speed","balanced") else "llama3-70b-8192"
    prompt = (
