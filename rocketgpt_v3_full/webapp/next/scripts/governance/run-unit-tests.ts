...
function testCrpsDeterministic() {
  const nodesA = [
    sampleNode({ cat_id: 'CAT-2', node_id: '2' }),
    sampleNode({ cat_id: 'CAT-1', node_id: '1' }),
  ]
  const nodesB = [
    sampleNode({ cat_id: 'CAT-1', node_id: '1' }),
    sampleNode({ cat_id: 'CAT-2', node_id: '2' }),
...
function testPolicyTrigger() {
  const crps = computeCrpsSignature({
    ...
    nodes: [
      sampleNode({
        purpose: 'legal security incident response',
        allowed_side_effects: ['workflow_dispatch'],
      }),
    ],
    params: { force: true },
  ...
  assert.ok(
    decision.containmentLevel >= 2,
    'Expected L2+ containment for high-risk policy conditions.',
  )
}
...
function testRedactionUtility() {
  ...
  const fp2 = createParamsFingerprint({
    safe: 'value',
    nested: { safe: 'ok', token: 't', apiKey: 'key' },
    password: 'abc',
  })
