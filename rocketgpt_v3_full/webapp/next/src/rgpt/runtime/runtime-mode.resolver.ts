...
export function resolveRuntimeMode(input: ResolveInput, cfg: RuntimeModeConfig): ResolveResult {
  ...
  if (input.triggers && input.triggers.length > 0) {
    ...
    const mapped = input.triggers.map((t) => cfg.downgradeTriggers[t]).filter(Boolean)

    if (mapped.includes('SAFE')) {
      return {
        mode: 'SAFE',
        reason: 'Downgraded due to triggers (SAFE).',
        warnings,
        blockedTransitions,
      }
    }
    if (mapped.includes('SUPERVISED')) {
      return {
        mode: 'SUPERVISED',
        reason: 'Downgraded due to triggers (SUPERVISED).',
        warnings,
        blockedTransitions,
      }
    }
    ...
    return {
      mode: 'SAFE',
      reason: 'Downgraded due to triggers (fallback SAFE).',
      warnings,
      blockedTransitions,
    }
  }
  ...
  if (isLocked(candidate)) {
    ...
    return {
      mode: 'SAFE',
      reason: 'Locked mode requested; forced SAFE.',
      warnings,
      blockedTransitions,
    }
  }
  ...
  if (candidate !== current && !allowed.includes(candidate)) {
    ...
    return {
      mode:
        current === 'SELF_EVOLUTION' ? 'SAFE' : (current as Exclude<RuntimeMode, 'SELF_EVOLUTION'>),
