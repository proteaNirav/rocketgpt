...
const ORCH_BASE_URL = process.env.RGPT_ORCH_BASE_URL ?? 'http://localhost:3000'
...
async function enableSafeMode(request: APIRequestContext, reason = 'Playwright Safe-Mode test') {
  const response = await request.post(`${ORCH_BASE_URL}${SAFE_MODE_ENABLE_ROUTE}`, {
    data: { reason },
  })
...
async function disableSafeMode(request: APIRequestContext) {
  const response = await request.post(`${ORCH_BASE_URL}${SAFE_MODE_DISABLE_ROUTE}`, {
    data: {},
  })
...
test.skip('should block builder/execute-all when Safe-Mode is enabled', async ({ request }) => {
  // 1) Enable Safe-Mode
  ...
  const execResponse = await request.post(`${ORCH_BASE_URL}${EXECUTE_ALL_ROUTE}`, {
    data: {},
  })
