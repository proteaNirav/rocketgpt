...
function normalizeDynamicCat(candidate: unknown): DynamicCatItem | null {
  ...
  const safeEffects: SideEffect[] = Array.isArray(item.allowed_side_effects)
    ? item.allowed_side_effects.filter((effect): effect is SideEffect =>
        effect === "none" || effect === "read_only" || effect === "ledger_write" || effect === "workflow_dispatch"
      )
    : ["read_only"];
...
export function loadDynamicCats(): DynamicCatItem[] {
  ...
  try {
    const raw = window.localStorage.getItem(DYNAMIC_CATS_KEY) || window.localStorage.getItem(LEGACY_DYNAMIC_CATS_KEY);
    if (!raw) return [];
...
export function makeDevPendingCat(input: {
  ...
  return {
    ...
    allowed_side_effects: input.allowed_side_effects?.length ? input.allowed_side_effects : ["read_only"],
