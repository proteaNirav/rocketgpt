...
class ApprovalStore {
  ...
  constructor(options?: ApprovalStoreOptions) {
    this.maxItems = options?.maxItems && options.maxItems > 0 ? options.maxItems : DEFAULT_MAX_ITEMS
  ...
  private enforceLimits(): void {
    ...
    const items = Array.from(this.store.values()).sort((a, b) => a.timestamp - b.timestamp)
