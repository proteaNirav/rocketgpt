...
export function createPendingCatsFromText(conversationText: string) {
  ...
  if (isEmail) {
    cats.push(
      makeDevPendingCat({
        name: 'Zoho Mail Connector',
        purpose: 'Connect to Zoho Mail, list/search messages (development pending).',
        tags: ['zoho', 'email', 'auth'],
    ...
    cats.push(
      makeDevPendingCat({
        ...
        purpose: 'Classify unnecessary emails using rules/heuristics (development pending).',
        tags: ['email', 'classifier', 'cleanup'],
        allowed_side_effects: ['read_only', 'ledger_write'],
      }),
    )
    cats.push(
      makeDevPendingCat({
        name: 'Bulk Delete Executor',
        purpose: 'Delete selected emails (moves to trash) with audit logs (development pending).',
        tags: ['email', 'delete', 'batch'],
        allowed_side_effects: ['workflow_dispatch', 'ledger_write'],
      }),
    )
  }
  if (isEmail && isRetention) {
    cats.push(
      makeDevPendingCat({
        name: 'Trash Purger',
        purpose: 'Permanently purge trash/recycle bin on retention schedule (development pending).',
        tags: ['email', 'trash', 'retention'],
        allowed_side_effects: ['workflow_dispatch', 'ledger_write'],
      }),
    )
  }
  if (isAutomation || isRetention) {
    cats.push(
      makeDevPendingCat({
        ...
        purpose: 'Schedule automated runs and produce evidence artifacts (development pending).',
        tags: ['scheduler', 'ops', 'audit'],
        allowed_side_effects: ['ledger_write'],
      }),
    )
  }
