import { LLMRouterRequest, LLMResponse, LLMProviderId, LLMError, LLMRetryOptions } from './types'
...
async function withRetries<T>(
  ...
  throw lastError instanceof Error ? lastError : new LLMError('Unknown error in withRetries')
}
...
export async function callLLM(
  ...
  return withRetries<LLMResponse>(async () => {
    switch (provider) {
      case 'openai':
