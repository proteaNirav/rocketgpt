...
function normalizeInitParams(initParams: unknown): {
  ok: boolean
  ...
  if (typeof initParams !== 'object' || initParams === null || Array.isArray(initParams))
    return { ok: false }
...
export function validateWorkflow(
  ...
  const firstExecutionIndex = workflow.nodes.findIndex(
    (node) => inferCategory(node, seedById) !== 'governance',
  )
  ...
  workflow.nodes.forEach((node, index) => {
    ...
    if (category === 'reporting') {
      const hasPriorAnalyticsLike = workflow.nodes.slice(0, index).some((priorNode) => {
  ...
  if (anyElevated) {
    pushWarning({
      ...
      message:
        'At least one node requests workflow_dispatch side-effect. Elevated governance is required.',
    })
    signals.push({
