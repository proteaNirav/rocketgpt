...
export async function listCats(input: { tenantId: string; page: number; pageSize: number }): Promise<{ items: CatRow[]; total: number }> {
...
export async function createCat(input: {
  ...
  if (isInMemoryEnabled()) {
    const state = readState();
    const dupe = state.cats.find((c) => c.tenant_id === input.tenantId && c.name.toLowerCase() === input.name.toLowerCase());
...
export async function updateCat(input: {
  ...
  if (isInMemoryEnabled()) {
    const state = readState();
    const row = state.cats.find((c) => c.tenant_id === input.tenantId && c.id === input.catId) || null;
    if (!row) return null;
...
export async function transitionCat(input: { tenantId: string; catId: string; status: CatsStatus }): Promise<CatRow | null> {
  if (isInMemoryEnabled()) {
    const state = readState();
    const row = state.cats.find((c) => c.tenant_id === input.tenantId && c.id === input.catId) || null;
    if (!row) return null;
...
export async function listVersions(tenantId: string, catId: string): Promise<CatVersionRow[]> {
  if (isInMemoryEnabled()) {
    ...
    if (!cat) return [];
    return state.versions.filter((v) => v.cat_id === catId).sort((a, b) => b.created_at.localeCompare(a.created_at));
  }
  const supabase = getSupabaseAdminClient();
