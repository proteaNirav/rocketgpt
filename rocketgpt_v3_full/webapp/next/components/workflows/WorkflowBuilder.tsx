...
import { WorkflowNode, WorkflowRunHistoryRecord, WorkflowStepResult } from '@/lib/workflow-types'
...
function isWorkflowNodeArray(value: unknown): value is WorkflowNode[] {
  ...
  return value.every((item) => {
    ...
    return (
      ...
      typeof candidate.cat_id === 'string' &&
...
function loadRunHistory(): WorkflowRunHistoryRecord[] {
  ...
  try {
    const parsed = JSON.parse(
      window.localStorage.getItem(WORKFLOW_RUNS_KEY) || '[]',
    ) as WorkflowRunHistoryRecord[]
...
export default function WorkflowBuilder() {
  ...
  async function runWorkflow(): Promise<void> {
    ...
    const runResponse = await executeWorkflowRun({
      ...
      callbacks: {
        onStepUpdate: (stepResult) => {
          ...
          if (
            (stepResult.status === 'success' || stepResult.status === 'failed') &&
            !stepStatusNotifiedRef.current.has(stepResult.stepId)
          ) {
    ...
    const nextEmails = [
      storedEmail,
      ...loadEmailOutbox().filter((item) => item.id !== storedEmail.id),
    ]
  ...
  const resultsOrdered = useMemo(() => {
    ...
    return [...results].sort(
      (a, b) => (indexMap.get(a.stepId) ?? 0) - (indexMap.get(b.stepId) ?? 0),
    )
  }, [nodes, results])
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 rounded-xl border border-gray-200 p-3 dark:border-neutral-800">
        <div className="space-y-1">
          ...
          <p className="text-xs text-gray-600 dark:text-gray-300">
            Draft key: <code>{WORKFLOW_DRAFT_KEY}</code>
          </p>
        </div>
      ...
      <section className="rounded-xl border border-gray-200 p-4 dark:border-neutral-800">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button
            type="button"
            onClick={() => {
              downloadJson('workflow-results.json', {
                workflowId,
                runId: activeRunId,
        ...
        <div className="mt-2 space-y-2">
          {runHistory.length === 0 ? (
            ...
            runHistory.map((entry) => (
              <div
                key={entry.runId}
                className="rounded border border-gray-200 p-2 text-xs dark:border-neutral-700"
              >
                <p className="font-mono">{entry.runId}</p>
                <p className="text-gray-600 dark:text-gray-300">{entry.createdAt}</p>
                <p>
                  {entry.run.results.filter((item) => item.status === 'success').length} success /{' '}
                  {entry.run.results.filter((item) => item.status === 'failed').length} failed
                </p>
              </div>
            ))
          )}
        ...
        >
          <div className="w-full max-w-3xl rounded-xl border border-gray-300 bg-white p-4 shadow-lg dark:border-neutral-700 dark:bg-neutral-950">
            ...
            <pre className="max-h-[65vh] overflow-auto rounded border border-gray-200 bg-gray-50 p-3 text-[12px] dark:border-neutral-800 dark:bg-neutral-900/40">
              {JSON.stringify(
                resultsOrdered.find((item) => item.stepId === viewJsonResultId)?.outputJson ?? {},
                null,
                2,
              )}
            </pre>
