...
function buildDraft(
  ...
  return {
    ...
    tags: Array.from(
      new Set([domain || 'dynamic', risk, ...keywordTags(goal), 'dynamic', 'development_pending']),
    ),
...
export default function CatsGenerator() {
  ...
  const draft = useMemo(
    () => buildDraft(goal, domain, risk, variation),
    [domain, goal, risk, variation],
  )
      ...
      <section className="rounded-xl border border-gray-200 p-4 dark:border-neutral-800">
        ...
        {!didGenerate ? (
          <p className="mt-2 text-sm text-gray-600 dark:text-gray-300">
            Generate to preview CAT draft JSON.
          </p>
        ) : (
          <>
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              ...
              <button
                type="button"
                onClick={() =>
                  downloadJson(`${draft.canonical_name.replaceAll('/', '-')}.json`, draft)
                }
                className="rounded border border-gray-300 px-3 py-1.5 text-sm"
              >
                Download JSON
            ...
            {savedCatId ? (
              <p className="mt-2 text-sm text-green-700 dark:text-green-300">Saved {savedCatId}.</p>
            ) : null}
          </>
        )}
      </section>
