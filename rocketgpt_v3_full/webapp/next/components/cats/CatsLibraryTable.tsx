...
function sortRows(
  ...
  return [...rows].sort((a, b) =>
    compareValues(String(a[column] ?? ''), String(b[column] ?? ''), direction),
  )
}
...
export default function CatsLibraryTable() {
  ...
  const range = useMemo(
    () => pageRange(sortedRows.length, page, pageSize),
  ...
  async function copyReplayCommand(item: CatCatalogItem): Promise<void> {
    ...
    try {
      ...
      window.setTimeout(
        () => setCopiedCatId((current) => (current === item.cat_id ? null : current)),
        1200,
      )
    } catch {
      publishNotification({
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 text-sm text-gray-600 dark:text-gray-300">
        ...
        <span>
          {filteredRows.length} of {allCats.length} catalog items
        </span>
      </div>
      ...
      <div className="overflow-x-auto rounded-xl border border-gray-200 dark:border-neutral-800">
        <table className="min-w-[1450px] w-full text-left text-sm">
          <thead className="bg-gray-100 text-xs uppercase tracking-wide text-gray-700 dark:bg-neutral-900 dark:text-gray-300">
            <tr>
              ...
              <th className="p-2">
                ...
                >
                  Canonical {sortIndicator('canonical_name')}
                </button>
              </th>
              <th className="p-2">
                <button
          ...
          <tbody>
            {pagedRows.length === 0 ? (
              <tr>
                <td colSpan={12} className="p-4 text-sm text-gray-600 dark:text-gray-300">
              ...
              pagedRows.map((item) => (
                ...
                >
                  ...
                  <td className="p-2">
                    <span
                      className={`inline-flex rounded-full border px-2 py-0.5 text-xs font-medium ${statusBadgeClass(item.status)}`}
                    >
                      {item.status}
                    </span>
                  </td>
