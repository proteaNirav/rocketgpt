...
export default function CreateCatWizard() {
  ...
  const errors = useMemo(() => {
    ...
    if (!canonical.trim()) {
      ...
      allCats.some((item) => item.canonical_name.toLowerCase() === canonical.trim().toLowerCase())
    ) {
        ...
        <section className="space-y-3 rounded-xl border border-gray-200 p-4 dark:border-neutral-800">
          ...
          <label className="block text-sm">
            ...
            <input
              ...
              className={`mt-1 w-full rounded border px-3 py-2 ${errors.name ? 'border-red-400' : 'border-gray-300'}`}
            />
          ...
          <label className="block text-sm">
            ...
            <select
              ...
              className="mt-1 w-full rounded border border-gray-300 px-3 py-2 dark:border-neutral-700 dark:bg-neutral-900"
            >
          ...
          <label className="block text-sm">
            ...
            />
            {errors.canonical ? (
              <span className="text-xs text-red-600">{errors.canonical}</span>
            ) : null}
          </label>
          <label className="block text-sm">
            Purpose *
            <textarea
              ...
              rows={3}
              className={`mt-1 w-full rounded border px-3 py-2 ${errors.purpose ? 'border-red-400' : 'border-gray-300'}`}
            />
      ...
      {step === 2 ? (
        <section className="space-y-3 rounded-xl border border-gray-200 p-4 dark:border-neutral-800">
          ...
          <label className="block text-sm">
            ...
            <select
              value={status}
              onChange={(event) => setStatus(event.target.value as CatCatalogItem['status'])}
              className="mt-1 w-full rounded border border-gray-300 px-3 py-2 dark:border-neutral-700 dark:bg-neutral-900"
            >
              {(['proposed', 'draft', 'approved', 'blocked', 'deprecated'] as const).map(
                (option) => (
                  <option key={option} value={option}>
                    {option}
                  </option>
          ...
          <div className="space-y-1 text-sm">
            ...
            <div className="flex flex-wrap gap-2">
              {(['read_only', 'ledger_write', 'workflow_dispatch', 'none'] as SideEffect[]).map(
                (effect) => (
                  <label
                    key={effect}
            ...
            {errors.sideEffects ? (
              <span className="text-xs text-red-600">{errors.sideEffects}</span>
            ) : null}
          </div>
          ...
          <label className="block text-sm">
            ...
            <select
              value={passportMode}
              onChange={(event) => setPassportMode(event.target.value as PassportMode)}
              className="mt-1 w-full rounded border border-gray-300 px-3 py-2 dark:border-neutral-700 dark:bg-neutral-900"
            >
        ...
        <section className="space-y-3 rounded-xl border border-gray-200 p-4 dark:border-neutral-800">
          ...
          {savedCatId ? (
            <p className="text-sm text-green-700 dark:text-green-300">
              Saved {savedCatId} to localStorage.
            </p>
          ) : null}
        </section>
      ) : null}
