...
export default function CatsReportsDashboard() {
  ...
  useEffect(() => {
    const onStorage = (event: StorageEvent) => {
      if (
        !event.key ||
        event.key === 'rgpt.cats.dynamic.v1' ||
        event.key === CATS_USAGE_KEY ||
        event.key === WORKFLOW_RUNS_KEY
      ) {
  ...
  const runUsage = useMemo(() => {
    const cutoff =
      usageWindowDays === null ? null : Date.now() - usageWindowDays * 24 * 60 * 60 * 1000
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <div className="flex gap-2">
          <button
            type="button"
            onClick={() => downloadJson('cats-report.json', reportPayload)}
              ...
              void exportXlsx()
                .then(() =>
                  publishNotification({
                    ...
                    message: 'CATS report XLSX exported.',
                  }),
                )
                .catch(() =>
                  publishNotification({
                    level: 'error',
                    title: 'Export Failed',
                    message: 'Unable to export CATS report XLSX.',
                  }),
                )
            }
            className="rounded border border-gray-300 px-3 py-1.5 text-sm"
      ...
      <section className="grid grid-cols-1 gap-3 md:grid-cols-5">
        {[
          { label: 'Total cats', value: summary.total },
          { label: 'Seed count', value: summary.seed },
          { label: 'Dynamic count', value: summary.dynamic },
