...
export default function CatsGeneratorWizard() {
  ...
  const validationError = useMemo(() => {
    ...
    if (!canonicalName.trim() || !canonicalName.includes('/'))
      return 'canonical_name must be namespace/name.'
    if (!entrypoint.trim()) return 'entrypoint is required.'
    if (selectedEffects.length === 0)
      return 'allowed_side_effects must include at least one explicit value.'
    return null
        ...
        <div className="mt-3 grid grid-cols-1 gap-3 md:grid-cols-2">
          ...
          <label className="text-sm md:col-span-2">
            ...
            <textarea
              ...
              className="h-20 w-full rounded border border-gray-300 px-2 py-1.5 dark:border-neutral-700 dark:bg-neutral-900"
            />
          </label>
          <label className="text-sm">
      ...
      <div className="rounded-xl border border-gray-200 p-4 dark:border-neutral-800">
        ...
        <div className="mt-3 grid grid-cols-1 gap-3 md:grid-cols-2">
          ...
          <label className="text-sm">
            ...
            <select
              value={owner}
              onChange={(e) => setOwner(e.target.value as CatOwner)}
              className="w-full rounded border border-gray-300 px-2 py-1.5 dark:border-neutral-700 dark:bg-neutral-900"
            >
          ...
          <label className="text-sm md:col-span-2">
            <div className="mb-1 text-xs text-gray-600 dark:text-gray-300">
              allowed_side_effects (explicit, required)
            </div>
            <div className="flex flex-wrap gap-2">
              {SIDE_EFFECTS.map((effect) => (
                <label
                  key={effect}
                  className="inline-flex items-center gap-1 rounded border border-gray-300 px-2 py-1 text-xs dark:border-neutral-700"
                >
                  <input
                    type="checkbox"
      ...
      <div className="rounded-xl border border-gray-200 p-4 dark:border-neutral-800">
        ...
        <div className="mt-4 space-y-2">
          ...
          <div className="rounded border border-gray-200 bg-gray-50 p-2 text-xs dark:border-neutral-700 dark:bg-neutral-950/40">
            bundle digest (sha256 over downloadable JSON bytes):{' '}
