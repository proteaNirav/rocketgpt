  ...
  if (!parts) {
    return (
      <pre className="bg-black text-white text-xs p-2 rounded overflow-auto">
        Diff parsing failed. Showing raw text.
        ...
        <div className="bg-black text-white rounded p-2 max-h-[350px] overflow-auto">
          {parts.map((p: any, i: number) => {
            ...
            return (
              <pre
                key={i}
                className={p.removed ? "bg-red-900 p-1 rounded whitespace-pre-wrap" : "whitespace-pre-wrap"}
        ...
        <div className="bg-black text-white rounded p-2 max-h-[350px] overflow-auto">
          {parts.map((p: any, i: number) => {
            ...
            return (
              <pre
                key={i}
                className={p.added ? "bg-green-900 p-1 rounded whitespace-pre-wrap" : "whitespace-pre-wrap"}
                dangerouslySetInnerHTML={{ __html: highlighted }}
              ></pre>
            );
          })}
