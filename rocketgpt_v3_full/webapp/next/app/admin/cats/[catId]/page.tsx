...
export default function AdminCatDetailPage() {
  ...
  const canEditMetadata = useMemo(() => cat?.status === "draft" || cat?.status === "review", [cat?.status]);

  async function load(targetCatId: string) {
    setLoading(true);
    setError(null);
    try {
      const [catResponse, versionsResponse] = await Promise.all([getCat(targetCatId), listCatVersions(targetCatId)]);
      setCat(catResponse);
        ...
        <>
          <section className="space-y-2 rounded-md border p-4">
            ...
            <div className="grid gap-2 md:grid-cols-2">
              <div>
                ...
                <Input value={name} onChange={(e) => setName(e.target.value)} disabled={!canEditMetadata} />
              </div>
              <div>
                <label className="mb-1 block text-sm">Description</label>
                <Input value={description} onChange={(e) => setDescription(e.target.value)} disabled={!canEditMetadata} />
              </div>
            </div>
          ...
          <section className="space-y-2 rounded-md border p-4">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <Button onClick={moveLifecycle} disabled={TRANSITION_OPTIONS[cat.status].length === 0}>
                Apply Transition
              </Button>
            </div>
          </section>
          ...
          <section className="space-y-2 rounded-md border p-4">
            ...
            <div className="grid gap-2 md:grid-cols-2">
              <Input value={publishVersionText} onChange={(e) => setPublishVersionText(e.target.value)} placeholder="1.0.0" />
              <Input value={publishCommandBundle} onChange={(e) => setPublishCommandBundle(e.target.value)} placeholder="cats/bundles/x" />
            </div>
            <div className="grid gap-2 md:grid-cols-2">
              <textarea
            ...
            <p className="text-xs text-muted-foreground">Publish is allowed in draft or approved states for this v1.</p>
          </section>

          <section className="space-y-2 rounded-md border p-4">
            <h2 className="font-medium">Versions</h2>
            {versions.length === 0 ? <p className="text-sm text-muted-foreground">No versions published yet.</p> : null}
            <div className="space-y-3">
              {versions.map((version) => (
                <div key={version.catVersionId} className="rounded-md border p-3">
                  ...
                  <p className="mt-2 text-xs text-muted-foreground">Command bundle: {version.commandBundleRef || "-"}</p>
                  <details className="mt-2">
                    <summary className="cursor-pointer text-xs text-blue-600">View manifest / rulebook</summary>
