...
export default function PlannerPage() {
  ...
  const [builderLoadingIndex, setBuilderLoadingIndex] = useState<number | null>(null)
    ...
    try {
      ...
      if (!res.ok) {
        try {
          ...
          setRawError(json)
          const msg = json?.message || json?.error?.message || `Planner API failed (${res.status})`
          setError(msg)
        } catch {
    ...
    try {
      ...
      if (!res.ok) {
        try {
          ...
          const msg = json?.message || json?.error?.message || `Builder API failed (${res.status})`
          setBuilderError(msg)
        } catch {
  ...
  const activeStep =
    builderStepIndex != null && steps[builderStepIndex] ? steps[builderStepIndex] : null
  ...
  return (
    <div className="p-6 space-y-4 text-gray-900 dark:text-gray-100">
      ...
      <p className="text-sm text-gray-600 dark:text-gray-400">
        Enter a software goal below. RocketGPT will call <code>/api/planner</code> to create a plan,
        and you can run individual steps via the Builder API.
      </p>
      ...
      {plan && (
        ...
        >
          ...
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-sm font-semibold">{plan.plan_title ?? 'Plan'}</h2>
              {plan.goal_summary && (
                <p className="text-xs text-gray-600 dark:text-gray-400">{plan.goal_summary}</p>
              )}
            </div>
            {plannerResult?.model && (
              ...
              >
                <div className="flex items-center justify-between gap-3">
                  <div>
                    ...
                    <div className="font-semibold">{step.title ?? 'Step'}</div>
                  </div>
                  <button
                    ...
                    "
                  >
                    {builderLoadingIndex === idx ? 'Running Builder…' : 'Run Builder'}
                  </button>
                </div>
      ...
      {builderResult?.result && activeStep && (
        ...
        >
          ...
          {builderResult.result.summary && (
            <p className="text-sm font-medium">{builderResult.result.summary}</p>
          )}

          {builderResult.result.details && (
            <pre className="mt-2 text-xs whitespace-pre-wrap">{builderResult.result.details}</pre>
          )}
