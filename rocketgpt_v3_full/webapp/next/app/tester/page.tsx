...
function buildTestRowsFromTester(tester: AnyRecord | null): TestRow[] {
  ...
  if (Array.isArray(engine.tests) && engine.tests.length > 0) {
    return engine.tests.map((t: AnyRecord, idx: number) => {
      ...
      const name =
        t.test_case || t.name || (t.id ? `Test #${idx + 1} (${t.id})` : `Test #${idx + 1}`)
  ...
  const ok = engine.success ?? tester.success ?? (http.result === 'match' ? true : null)

  const duration = typeof engine.duration_ms === 'number' ? engine.duration_ms : null
 ...
 */
function deriveTesterTotalCases(tester: AnyRecord | null, rows: TestRow[]): number {
...
export default function TesterPage() {
  ...
  const orchTester: AnyRecord | null = (orchResp?.builder as AnyRecord | undefined)?.tester || null

  const orchOk =
    (typeof orchResp?.success === 'boolean' && orchResp.success) ||
    (typeof (orchResp?.builder as AnyRecord | undefined)?.success === 'boolean' &&
  ...
  return (
    <div className="p-6 space-y-8">
        ...
        <div className="rounded-xl border border-gray-800 p-6 bg-black/20 space-y-6">
          ...
          <div className="grid gap-4 md:grid-cols-3">
            ...
            <div className="border rounded-lg p-4 bg-black/30 border-gray-700 space-y-2">
              <div className="flex items-center justify-between">
                <span className="text-sm font-semibold text-gray-200">Overall</span>
                <StatusBadge ok={directOk} />
              </div>
              <p className="text-sm text-gray-300">
                <span className="font-semibold">Tests executed:</span> {directTotal}
              </p>
              {typeof directTester?.engine?.tests_passed === 'number' && (
                <p className="text-sm text-gray-300">
                  <span className="font-semibold">Passed:</span> {directTester.engine.tests_passed}{' '}
                  &nbsp;|&nbsp;
                  <span className="font-semibold">Failed:</span> {directTester.engine.tests_failed}
                </p>
              )}
              {typeof directTester?.engine?.duration_ms === 'number' && (
                <p className="text-sm text-gray-300">
                  <span className="font-semibold">Duration:</span> {directTester.engine.duration_ms}{' '}
                  ms
                </p>
              )}
            ...
            <div className="border rounded-lg p-4 bg-black/30 border-gray-700 space-y-1">
              <div className="flex items-center justify-between">
                <span className="text-sm font-semibold text-gray-200">Profile</span>
              ...
              <p className="text-sm text-gray-300">
                <span className="font-semibold">Label:</span> {directProfile?.label ?? 'Base Smoke'}
              </p>
              {directProfile && (
                <>
                  <p className="text-sm text-gray-300">
                    <span className="font-semibold">Strictness:</span> {directProfile.strictness}
                  </p>
                  <p className="text-sm text-gray-300">
                    <span className="font-semibold">Depth:</span> {directProfile.depth}
                  </p>
                  <p className="text-xs text-gray-400 mt-2">
                    Max cases: {directProfile.maxTestCases} &nbsp;|&nbsp; Max duration:{' '}
                    {directProfile.maxDurationMs} ms &nbsp;|&nbsp; Parallelism:{' '}
                    {directProfile.parallelism}
                  </p>
                </>
              )}
            </div>
            ...
            <div className="border rounded-lg p-4 bg-black/30 border-gray-700 space-y-1">
              <div className="flex items-center justify-between">
                <span className="text-sm font-semibold text-gray-200">HTTP Evaluation</span>
                <StatusBadge ok={directHttp?.result === 'match' ? true : null} />
              </div>
              <p className="text-sm text-gray-300">
                <span className="font-semibold">Status Code:</span> {directHttp?.status_code ?? '-'}
              </p>
              <p className="text-sm text-gray-300">
                <span className="font-semibold">Category:</span> {directHttp?.category ?? '-'}
              </p>
              {directHttp?.message && (
                <p className="text-xs text-gray-400 mt-2 whitespace-pre-line">
                  {directHttp.message}
            ...
            <div className="overflow-x-auto border border-gray-700 rounded-lg">
              <table className="w-full border-collapse text-sm">
                <thead className="bg-gray-900/60">
                  <tr>
                    <th className="border border-gray-700 px-3 py-2 text-left">Test Case</th>
                    <th className="border border-gray-700 px-3 py-2 text-left">Status</th>
                    <th className="border border-gray-700 px-3 py-2 text-left">Duration (ms)</th>
                  </tr>
                </thead>
                <tbody>
                  {directRows.map((row, idx) => (
                    <tr key={idx} className="bg-black/40 hover:bg-black/60">
                      <td className="border border-gray-800 px-3 py-2">{row.name}</td>
                      <td className="border border-gray-800 px-3 py-2">
                        <StatusBadge ok={row.ok} />
                      </td>
                      <td className="border border-gray-800 px-3 py-2">{row.duration ?? '-'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <div className="border border-gray-700 rounded-lg p-4 bg-black/40">
            ...
            <div className="max-h-48 overflow-y-auto text-gray-400 whitespace-pre-wrap bg-black/30 p-3 rounded-md text-xs">
              {directLogs.length > 0 ? directLogs.join('\n') : 'No logs returned.'}
            </div>
          </div>
            ...
            {directArtifacts.length > 0 ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                {directArtifacts.map((a, idx) => (
                  <div key={idx} className="rounded-lg border border-gray-700 bg-black/30 p-3">
                    ...
                    {a.type && typeof a.type === 'string' && a.type.startsWith('image/') ? (
                      ...
                      <div className="text-gray-400 text-xs mb-3">({a.type || 'file'})</div>
                    )}
                    <a
                      href={a.url}
                      target="_blank"
              ...
              <div className="text-gray-500 text-xs">No artifacts generated.</div>
            )}
          </div>
      ...
      <div className="rounded-xl border border-gray-800 p-6 bg-black/30">
        <h2 className="text-xl font-semibold mb-4">Orchestrator → Builder → Tester Pipeline</h2>
          ...
          <div className="mt-6 space-y-4">
            ...
            <div className="border border-gray-700 rounded-lg p-4 bg-black/40">
              <div className="flex items-center justify-between mb-1">
                <span className="text-sm font-semibold text-gray-200">Pipeline Summary</span>
                <StatusBadge ok={orchOk ? true : null} />
              </div>
              <p className="text-sm text-gray-300">
                <span className="font-semibold">Message:</span> {orchResp.message || '(no message)'}
              </p>
              {orchResp.runId && (
                <p className="text-xs text-gray-400 mt-1">Run ID: {orchResp.runId}</p>
              )}
            </div>
            ...
            {orchTester && (
              <div className="border border-gray-700 rounded-lg p-4 bg-black/40 space-y-3">
                ...
                <p className="text-sm text-gray-300">
                  ...
                  {orchTester.status ||
                    (orchTesterOk === true ? 'success' : orchTesterOk === false ? 'failed' : '-')}
                </p>
                {orchTester.summary && (
                  <p className="text-xs text-gray-400 whitespace-pre-line">{orchTester.summary}</p>
                )}
