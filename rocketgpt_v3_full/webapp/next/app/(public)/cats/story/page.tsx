...
export default function CatsStoryPage() {
      ...
      return suggestions.map((suggestion, index) => {
        ...
        if (!existing) {
          ...
          const nextNode: WorkflowNode = {
            ...
            expected_outputs: [
              { id: `${suggestion.item.cat_id}-out-1`, label: "Output is deterministic", checked: false },
              { id: `${suggestion.item.cat_id}-out-2`, label: "No policy violations", checked: false },
              { id: `${suggestion.item.cat_id}-out-3`, label: "Evidence generated", checked: false },
            ],
          };
  ...
  const artifactForValidation = useMemo<WorkflowArtifact>(
    () => ({
      ...
      side_effects_summary: {
        union: Array.from(new Set(nodesForValidation.flatMap((node) => node.allowed_side_effects))).sort() as WorkflowArtifact["side_effects_summary"]["union"],
        includes_workflow_dispatch: nodesForValidation.some((node) => node.allowed_side_effects.includes("workflow_dispatch")),
      },
    }),
  ...
  function createWorkflow() {
    ...
    router.push(`/workflows/builder?from=story&draftId=${encodeURIComponent(nextArtifact.draft_id)}`);
  }
  ...
  return (
    <div className="space-y-4">
        ...
        <section className="rounded-xl border border-gray-200 p-4 dark:border-neutral-800">
          ...
          <div className="mt-3 h-[28rem] space-y-2 overflow-auto rounded border border-gray-200 bg-gray-50 p-3 dark:border-neutral-700 dark:bg-neutral-900/30">
            {messages.map((message) => (
              <div key={message.id} className="rounded border border-gray-200 bg-white p-2 text-sm dark:border-neutral-700 dark:bg-neutral-900">
        ...
        <section className="rounded-xl border border-gray-200 p-4 dark:border-neutral-800">
          ...
          <div className="mt-3 rounded-lg border border-gray-200 bg-gray-50 p-3 text-sm dark:border-neutral-700 dark:bg-neutral-900/40">
            ...
            <div className="mt-1 text-xs">
              {validation.errors.length} errors, {validation.warnings.length} warnings, {validation.signals.length} signals
            </div>
            <ul className="mt-2 space-y-1 text-xs">
              {validation.errors.slice(0, 2).map((issue) => (
                <li key={`${issue.code}-${issue.node_id || "global"}-e`} className="text-red-700 dark:text-red-300">
                  [E] {issue.message}
                </li>
              ))}
              {validation.warnings.slice(0, 2).map((issue) => (
                <li key={`${issue.code}-${issue.node_id || "global"}-w`} className="text-amber-700 dark:text-amber-300">
                  [W] {issue.message}
                </li>
              ))}
              ...
              >
                <div className="flex items-start justify-between gap-2">
                  <div>
                    ...
                    <div className="font-mono text-xs text-gray-600 dark:text-gray-300">{node.cat_id}</div>
                    <div className="text-xs text-gray-600 dark:text-gray-300">{node.canonical_name}</div>
                  </div>
                  <div className="flex gap-1">
                    <button onClick={() => setNodes((current) => moveNode(current, node.node_id, -1))} className="rounded border border-gray-300 px-2 py-1 text-xs dark:border-neutral-700">Up</button>
                    <button onClick={() => setNodes((current) => moveNode(current, node.node_id, 1))} className="rounded border border-gray-300 px-2 py-1 text-xs dark:border-neutral-700">Down</button>
                    <button onClick={() => setNodes((current) => current.filter((item) => item.node_id !== node.node_id))} className="rounded border border-red-300 px-2 py-1 text-xs text-red-700 dark:border-red-900 dark:text-red-300">Remove</button>
                    <button onClick={() => downloadJson(`${node.cat_id}-node.json`, node)} className="rounded border border-gray-300 px-2 py-1 text-xs dark:border-neutral-700">Node JSON</button>
                  </div>
                </div>
                ...
                <div className="mt-1 text-xs text-gray-700 dark:text-gray-200">
                  Side-effects: {node.allowed_side_effects.join(", ")} | Approval: {node.requires_approval ? "required" : "optional"} | Passport: {node.passport_required ? "required" : "optional"}
                </div>
                ...
                <label className="mt-2 block text-xs">
                  ...
                  <textarea
                    ...
                    onChange={(event) =>
                      updateNode(node.node_id, (current) => ({ ...current, expected_behavior: event.target.value }))
                    }
                ...
                <label className="mt-2 block text-xs">
                  Init params (JSON)
                  <textarea
                    value={initParamDrafts[node.node_id] ?? JSON.stringify(node.init_params, null, 2)}
                    onChange={(event) =>
                      setInitParamDrafts((current) => ({ ...current, [node.node_id]: event.target.value }))
                    }
                ...
                <div className="mt-1 flex items-center gap-2">
                  ...
                  {initParamErrors[node.node_id] ? (
                    <span className="text-xs text-red-700 dark:text-red-300">{initParamErrors[node.node_id]}</span>
                  ) : null}
                </div>
                <div className="mt-2">
                        ...
                        <input
                          ...
                          onChange={(event) =>
                            updateNode(node.node_id, (current) => ({
                              ...current,
                              expected_outputs: current.expected_outputs.map((item) =>
                                item.id === output.id ? { ...item, label: event.target.value } : item
                              ),
                            }))
                          }
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <button
          onClick={() => downloadJson("cats-story-workflow-draft.json", { ...artifact, nodes: nodesForValidation, validation })}
