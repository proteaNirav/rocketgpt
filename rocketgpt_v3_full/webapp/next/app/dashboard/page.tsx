...
/**
 * Dashboard page — system status and usage overview.
 * Moved from root to /dashboard for Phase-1 chat-first UX.
 */
export default function DashboardPage() {
  ...
  const primaryPlan: Plan | undefined =
    limits?.plans?.find((p) => p.plan_code === 'BRONZE') ?? (limits?.plans && limits.plans[0])

  const statusLabel = health?.ok === true ? 'Online' : health?.ok === false ? 'Degraded' : 'Unknown'
  ...
  return (
    <div className="flex flex-col gap-6 p-4 md:p-6">
      ...
      <div className="flex items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight">RocketGPT Overview</h1>
        ...
        <div className="grid gap-4 md:grid-cols-3">
          ...
          <div className="rounded-xl border border-border bg-card p-4">
            <div className="flex items-center justify-between">
              <div className="text-xs font-medium uppercase text-muted-foreground">Instance</div>
              {health?.env && (
            ...
            <div className="mt-2 text-lg font-semibold">{formatUptime(health?.startedAt)}</div>
            <div className="mt-1 text-xs text-muted-foreground">
              Uptime since{' '}
              {health?.startedAt ? new Date(health.startedAt).toLocaleString() : 'unknown'}
            </div>
            <div className="mt-3 text-[11px] text-muted-foreground break-all">
              Commit: {health?.commit ? health.commit.substring(0, 7) : 'n/a'}
            </div>
          </div>

          {/* Plan & limits */}
          <div className="rounded-xl border border-border bg-card p-4">
            <div className="text-xs font-medium uppercase text-muted-foreground">Plan & Limits</div>
            ...
            <div className="mt-3 space-y-1 text-xs text-muted-foreground">
              ...
              <div>
                Per day:{' '}
                <span className="font-semibold text-foreground">{primaryPlan?.per_day ?? '—'}</span>
              </div>
            </div>
          </div>
          ...
          >
            ...
            <div className="mt-3">
              ...
              <div className="mt-1 text-[11px] text-muted-foreground">
                This bar will evolve into a proper usage chart in a future iteration.
              </div>
            </div>
