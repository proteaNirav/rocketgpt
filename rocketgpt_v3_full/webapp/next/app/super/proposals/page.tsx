...
type ProposalRow = {
  ...
  // ðŸ‘‡ this is the column we added earlier in the SQL
...
export default function SuperProposalsPage() {
  ...
  return (
    <div className="p-5">
      <div className="mb-5">
        <h1 className="text-2xl font-semibold">Superuser â€” Proposals</h1>
        <p className="text-sm text-gray-600">
          Approve proposals to enqueue Self-Apply. Manual Review button appears when a job is
          created.
        </p>
      </div>
      ...
      {loading ? (
        <div className="text-sm text-gray-600">Loading proposalsâ€¦</div>
      ) : proposals.length === 0 ? (
            ...
            return (
              <div key={p.id} className="rounded-lg border border-gray-200 p-4 shadow-sm">
                    ...
                    <div className="mt-1 text-xs text-gray-600">
                      ...
                      {p.last_self_apply_job_id && (
                        <span className="ml-2 inline-flex items-center rounded bg-blue-50 px-2 py-0.5 text-blue-700">
                          Job: {p.last_self_apply_job_id.slice(0, 8)}â€¦
                        </span>
                      )}
                    </div>
                ...
                <div className="mt-3 flex flex-wrap items-center gap-2">
                  ...
                  {!!p.last_self_apply_job_id && (
                    <ManualReviewButton jobId={p.last_self_apply_job_id} proposalId={p.id} />
                  )}
                </div>
              </div>
            )
