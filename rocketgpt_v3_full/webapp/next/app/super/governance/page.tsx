...
export default function GovernanceAdminPage() {
  ...
  async function runDigestNow() {
    ...
    try {
      ...
      setMessage(
        json.ran ? `Digest generated: ${json.digestId}` : (json.reason ?? 'Digest not due.'),
      )
      await loadAll()
    } catch (error) {
    ...
    <div className="space-y-4 p-5">
      ...
      <section className="rounded border border-gray-200 p-3 dark:border-neutral-700">
        ...
        <div className="mt-2 space-y-2">
          {policies.map((rule) => (
            <div
              key={rule.id}
              className="rounded border border-gray-200 p-2 dark:border-neutral-800"
            >
          ...
          {policies.length === 0 ? (
            <div className="text-sm text-gray-600">No policy rules found.</div>
          ) : null}
        </div>
      </section>
      ...
      <section className="rounded border border-gray-200 p-3 dark:border-neutral-700">
        ...
        <div className="mt-2 space-y-2">
          {events.slice(0, 20).map((event) => (
            <div
              key={event.id}
              className="rounded border border-gray-200 p-2 text-sm dark:border-neutral-800"
            >
          ...
          {events.length === 0 ? (
            <div className="text-sm text-gray-600">No containment events yet.</div>
          ) : null}
        </div>
      </section>
      ...
      <section className="rounded border border-gray-200 p-3 dark:border-neutral-700">
        ...
        <div className="mt-2 space-y-2">
          ...
          {tasks.length === 0 ? (
            <div className="text-sm text-gray-600">No open foresight tasks.</div>
          ) : null}
        </div>
      </section>
      ...
      <section className="rounded border border-gray-200 p-3 dark:border-neutral-700">
        ...
        <div className="mt-2 space-y-2">
          ...
          {digests.length === 0 ? (
            <div className="text-sm text-gray-600">No digests published yet.</div>
          ) : null}
        </div>
      </section>
    </div>
