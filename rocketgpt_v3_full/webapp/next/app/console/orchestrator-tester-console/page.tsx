  ...
  return (
    <span
      className={
        ...
        (isSuccess ? 'bg-emerald-500/10 text-emerald-400' : 'bg-destructive/10 text-destructive')
      }
    >
      {status}
    </span>
  ...
  async function runPipeline() {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(`HTTP ${res.status} ${res.statusText} → ${text.substring(0, 300)}`)
      }
    ...
    <div className="px-6 py-8">
      <div className="mx-auto max-w-6xl space-y-6">
        ...
        <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
          <div>
            <h1 className="text-2xl font-semibold tracking-tight">Orchestrator → Tester Console</h1>
            <p className="mt-1 text-sm text-muted-foreground">
              Execute the orchestrator pipeline and inspect tester output, results, and logs.
            </p>
          </div>
        ...
        {!data && !error && !loading && (
          <p className="pt-4 text-center text-sm text-muted-foreground">
            Click <span className="font-semibold">Run Orchestrator → Tester</span> to execute the
            pipeline and see results here.
          </p>
        )}
          ...
          <div className="space-y-6">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="rounded-xl border bg-card p-6 text-sm shadow-sm">
                <h3 className="mb-2 text-sm font-semibold">Orchestrator Status</h3>
                <div className="space-y-1 text-sm">
                  ...
                  {data.builder?.message && (
                    <div className="text-muted-foreground">{data.builder.message}</div>
                  )}
                </div>
              </div>
                ...
                <div className="rounded-xl border bg-card p-6 text-sm shadow-sm">
                  ...
                  <div className="space-y-1 text-sm">
                    <div>
                      ...
                      <span className="font-mono text-xs">{tester.test_run_id}</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <span className="font-medium">Status:</span>
                      <StatusBadge status={tester.status} />
                    </div>
                    <div className="text-muted-foreground">{tester.summary}</div>
                  </div>
                </div>
              )}
            </div>
                      ...
                      <tbody>
                        {tester.results.map((r, idx) => (
                          <tr key={idx} className="border-b last:border-none text-sm">
                            <td className="px-3 py-2 font-mono text-xs">{r.test_case}</td>
            ...
            {!tester && !error && (
              <p className="pt-2 text-center text-sm text-muted-foreground">
                ...
                <span className="font-semibold">Run Orchestrator → Tester</span> to execute the
