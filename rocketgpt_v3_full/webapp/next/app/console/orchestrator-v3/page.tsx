...
const TIMELINE_ORDER: DerivedStatus[] = [
  "pending",
  "planner",
  "builder",
  "tester",
...
export default function OrchestratorV3ConsolePage() {
  ...
  const handleStartRun = async () => {
    try {
      ...
      if (!result.success || !result.runId) {
        appendLog(
          `[ERROR] start-run failed: ${result.message ?? "Unknown error"}`,
        );
        return;
      }
  ...
  const handleAutoAdvanceOnce = async () => {
    ...
    try {
      ...
      if (!resp.success) {
        appendLog(
          `[ERROR] auto-advance failed for run ${currentRunId}: ${
      ...
      try {
        ...
        if (status.success) {
          ...
          appendLog(
            `[WARN] status call failed for run ${currentRunId}: ${
      ...
      appendLog(
        `[ERROR] auto-advance exception for run ${currentRunId}: ${
  ...
  const handleAutoAdvanceUntilDone = async () => {
    ...
    try {
      ...
      while (safetyCounter < 30) {
        ...
        if (!resp.success) {
          appendLog(
            `[ERROR LOOP] auto-advance failed (phase=${phase}): ${
              resp.message ?? "Unknown error"
            }`,
          );
          break;
        ...
        try {
          ...
          appendLog(
            `[WARN] Could not refresh status during loop: ${
              (err as Error).message
            }`,
          );
        }
      ...
      appendLog(
        `[ERROR LOOP] auto-advance loop exception: ${(err as Error).message}`,
      );
    } finally {
      setIsBusy(false);
    }
      ...
      <div className="mx-auto max-w-5xl space-y-6">
          ...
          <h2 className="text-lg font-semibold flex items-center justify-between">
            ...
            {currentRunId && (
              <span className="text-xs font-normal text-slate-400">
                Current run ID:{" "}
                <span className="font-mono text-sky-400">{currentRunId}</span>
              </span>
            )}
          </h2>
        ...
        <section className="rounded-xl border border-slate-800 bg-slate-900/40 p-4 space-y-3">
          ...
          <p className="text-xs text-slate-400">
            ...
            <span className="font-mono">Planner → Builder → Tester → Finalize</span>. You
            can also run the full loop automatically.
          </p>
          <div className="flex flex-wrap items-center gap-3">
            ...
            {isBusy && (
              <span className="text-xs text-sky-400">
                Working… check logs and timeline below.
              </span>
            )}
          </div>
