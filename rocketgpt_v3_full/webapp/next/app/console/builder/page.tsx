...
export default function BuilderConsolePage() {
  ...
  const loadSteps = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/orchestrator/builder/list?runId=${encodeURIComponent(runId)}`, {
        method: 'GET',
        headers: {
      ...
      if (!res.ok) {
        setError(data?.message ?? data?.error ?? 'Failed to load builder steps for this run.')
        setSteps([])
        return
      }
  ...
  const executeNextStep = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) {
        setError(data?.message ?? data?.error ?? 'Failed to execute next builder step.')
        return
      }
  ...
  const executeAllSteps = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) {
        setError(data?.message ?? data?.error ?? 'Failed to execute all pending builder steps.')
        return
      }
      ...
      setError(err?.message ?? 'Unexpected error executing all pending builder steps.')
    } finally {
  ...
  return (
    <div className="p-6 space-y-6">
      <h1 className="text-2xl font-semibold mb-2">Orchestrator · Builder Console</h1>
        ...
        <table className="w-full text-sm border-collapse">
          <thead className="bg-neutral-900">
            <tr>
              ...
              <th className="px-3 py-2 text-left border-b border-neutral-800">Planner</th>
              <th className="px-3 py-2 text-left border-b border-neutral-800">Builder</th>
              <th className="px-3 py-2 text-left border-b border-neutral-800">Title</th>
          ...
          <tbody>
            {steps.length === 0 ? (
              ...
              steps.map((step, index) => (
                <tr key={step.id} className="odd:bg-neutral-950 even:bg-neutral-900/40">
                  <td className="px-3 py-2 border-b border-neutral-900">{index + 1}</td>
                  <td className="px-3 py-2 border-b border-neutral-900">{step.planner_step_no}</td>
                  <td className="px-3 py-2 border-b border-neutral-900">{step.builder_step_no}</td>
                  <td className="px-3 py-2 border-b border-neutral-900">{step.title}</td>
                  ...
                  <td className="px-3 py-2 border-b border-neutral-900 max-w-md">
                    <div className="line-clamp-3 whitespace-pre-wrap text-neutral-300">
                      {step.llm_output
                        ? step.llm_output.slice(0, 300) + (step.llm_output.length > 300 ? '…' : '')
                        : step.error_message
