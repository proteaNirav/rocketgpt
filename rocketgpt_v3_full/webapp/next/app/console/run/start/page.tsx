...
export default function StartRunPage() {
  const [goalTitle, setGoalTitle] = useState<string>('Add Dark Mode to Settings UI')
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        setError(data?.message ?? data?.error ?? 'Failed to start orchestration run.')
        setResult(null)
        return
      }
  ...
  return (
    <div className="p-6 space-y-6">
      <h1 className="text-2xl font-semibold mb-2">Orchestrator · Start Run</h1>
      <p className="text-sm text-gray-500 mb-4">
        Create a new orchestration run. This will call Planner to generate a plan, create Builder
        steps, and automatically execute all pending Builder steps for the run.
      </p>

      <form onSubmit={handleSubmit} className="space-y-4 border rounded-lg p-4 bg-neutral-900/40">
        ...
        <div className="flex flex-wrap gap-4">
          ...
          <div className="flex flex-col gap-1">
            ...
            <input
              ...
              onChange={(e) => setMaxBuilderSteps(Number(e.target.value) || 0)}
      ...
      {result && result.success && (
        <div className="space-y-4 border rounded-lg px-4 py-3 bg-neutral-900/40">
          ...
          <div className="text-sm space-y-1">
            ...
            <div>
              <span className="font-medium">Goal:</span> {result.goal_title}
            </div>
            {result.goal_description && (
              <div>
                <span className="font-medium">Description:</span> {result.goal_description}
              </div>
            )}
            <div>
              <span className="font-medium">Planner Model:</span> {result.planner_model}
            </div>
            <div>
              <span className="font-medium">Builder Model:</span> {result.builder_model}
            </div>
          </div>
          ...
          {result.planner && (
            <div className="text-sm space-y-1">
              ...
              <div>
                Steps: {result.planner.stepsCount} · Duration: {result.planner.durationMs} ms
              </div>
              <details className="mt-1">
                <summary className="cursor-pointer text-neutral-400">Show raw planner JSON</summary>
          ...
          {result.builder && (
            <div className="text-sm space-y-1">
              ...
              <div>
                ...
                {result.builder.remainingPending} · Duration: {result.builder.durationMs} ms
              </div>
              <details className="mt-1">
                <summary className="cursor-pointer text-neutral-400">Show executions</summary>
                <div className="mt-2 max-h-64 overflow-auto border border-neutral-800 rounded">
                  <table className="w-full text-xs border-collapse">
                    <thead className="bg-neutral-900">
                      <tr>
                        <th className="px-2 py-1 text-left border-b border-neutral-800">Step</th>
                        <th className="px-2 py-1 text-left border-b border-neutral-800">Status</th>
                        <th className="px-2 py-1 text-left border-b border-neutral-800">
                          Duration (ms)
                        </th>
                        <th className="px-2 py-1 text-left border-b border-neutral-800">Error</th>
                      </tr>
                    </thead>
                    <tbody>
                      {result.builder.executions?.map((ex) => (
                        <tr key={ex.stepId} className="odd:bg-neutral-950 even:bg-neutral-900/40">
                          <td className="px-2 py-1 border-b border-neutral-900">
                            {ex.planner_step_no}.{ex.builder_step_no}
                          </td>
                          <td className="px-2 py-1 border-b border-neutral-900">{ex.status}</td>
                          <td className="px-2 py-1 border-b border-neutral-900">{ex.durationMs}</td>
                          <td className="px-2 py-1 border-b border-neutral-900">
                            {ex.errorMessage ?? '—'}
                          </td>
                        </tr>
          ...
          {typeof result.totalDurationMs === 'number' && (
            <div className="text-sm">
              <span className="font-medium">Total Orchestrator Time:</span> {result.totalDurationMs}{' '}
              ms
            </div>
          )}
