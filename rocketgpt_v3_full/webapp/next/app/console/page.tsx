      ...
      <div className="flex-1 overflow-auto px-4 py-3 space-y-3 text-sm">
        {messages.length === 0 ? (
          <div className="mt-4 text-center text-xs text-slate-400">
            <p>No messages yet.</p>
            <p className="mt-1">
              Try a small test prompt to verify the layout and behaviour of the console.
            </p>
    ...
    return (
      <div className="fixed inset-0 z-30 flex flex-col bg-slate-950">
        <div className="flex items-center justify-between border-b border-slate-800 px-6 py-3">
          <div>
            <h1 className="text-lg font-semibold text-slate-50">RocketGPT Console</h1>
            <p className="text-xs text-slate-400">
              Focus mode hides navigation and side panels so you can work only with this console.
            </p>
          </div>
          <button
            type="button"
  ...
  return (
    <div className="flex flex-col gap-4 py-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold text-slate-100">RocketGPT Console</h1>
          <p className="text-xs md:text-sm text-slate-400">
            Three panel control room to experiment with prompts tools and routing.
          </p>
        </div>
      ...
      >
        {showTools && (
          <div className="rounded-lg border border-slate-700 bg-slate-950/60 p-4 text-xs text-slate-300">
            <h2 className="text-sm font-semibold text-slate-100">Engines and tools</h2>
            <div className="mt-3">
              <div className="text-[11px] font-semibold text-slate-200">Engines</div>
            ...
            <p className="mt-3 border-t border-slate-800 pt-3 text-[11px] text-slate-400">
              Later this panel will connect to real model routing tools and environments not only
              static placeholders.
            </p>
          </div>
        )}
        ...
        {showInspector && (
          <div className="rounded-lg border border-slate-700 bg-slate-950/60 p-4 text-xs text-slate-300">
            ...
            <p className="mt-2">
              This panel will later show request and response metadata for the current session.
            </p>
            ...
            <p className="mt-3 border-t border-slate-800 pt-3 text-[11px] text-slate-400">
              For now the layout is wired and safe for go live while the backend orchestration is
              still a placeholder.
            </p>
          </div>
        )}
