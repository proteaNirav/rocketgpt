...
function BuildResultView({ output }: { output: any }) {
  ...
  if (!output.files_changed || output.files_changed.length === 0) return <p>No files changed.</p>
        ...
        <div key={idx} className="border p-4 rounded bg-muted text-sm">
          ...
          {fc.diff && (
            <details className="mt-2">
              <summary className="cursor-pointer text-xs">Show Raw Diff</summary>
    ...
    <div className="space-y-4">
      {output.test_results.map((t: any, idx: number) => (
        <div key={idx} className="border p-3 rounded bg-muted">
          ...
          {t.error && (
            <pre className="bg-red-900 text-white p-2 rounded mt-2 text-xs">{t.error}</pre>
          )}
        </div>
      ))}
    </div>
...
function ArtifactsView({ artifacts }: { artifacts: any[] }) {
  if (!artifacts || artifacts.length === 0) return <p>No artifacts available.</p>
...
export default function OrchestratorConsole() {
  ...
  const downloadRunArtifactsJson = (run: RunRecord) => {
    const testArtifacts = Array.isArray(run.test?.artifacts) ? run.test.artifacts : []
    const releaseArtifacts = Array.isArray(run.release?.artifacts) ? run.release.artifacts : []
  ...
  useEffect(() => {
    try {
      const raw =
        typeof window !== 'undefined' ? window.localStorage.getItem('rgpt_run_history') : null
      if (raw) {
  ...
  useEffect(() => {
    try {
      if (typeof window !== 'undefined') {
        window.localStorage.setItem('rgpt_run_history', JSON.stringify(runHistory))
      }
    } catch (err) {
  ...
  if (selectedRunIndex !== null && selectedRunIndex >= 0 && selectedRunIndex < runHistory.length) {
    ...
    <div className="p-6 space-y-6">
      ...
      <Tabs defaultValue="plan">
        ...
        <TabsContent value="artifacts">
          <ArtifactsView artifacts={testOutput?.artifacts || releaseOutput?.artifacts || []} />
        </TabsContent>
                ...
                {!selectedRun ? (
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center justify-between mb-2">
                      <div>
                        <p className="font-semibold text-sm">{selectedRun.goal_title}</p>
                    ...
                    <details open>
                      <summary className="cursor-pointer font-semibold">Plan Output</summary>
                    ...
                    <details>
                      <summary className="cursor-pointer font-semibold">Build Output</summary>
                      <pre className="bg-black text-white p-2 rounded mt-1 overflow-auto">
                        {selectedRun.build
                    ...
                    <details>
                      <summary className="cursor-pointer font-semibold">Test Output</summary>
                      <pre className="bg-black text-white p-2 rounded mt-1 overflow-auto">
                        {selectedRun.test
                    ...
                    <details>
                      <summary className="cursor-pointer font-semibold">Release Output</summary>
                      <pre className="bg-black text-white p-2 rounded mt-1 overflow-auto">
                        {selectedRun.release
