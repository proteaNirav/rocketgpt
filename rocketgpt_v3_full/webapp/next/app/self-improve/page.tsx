...
type ProposalRow = {
  proposal: {
    ...
    plan: {
      scope: { allowed_paths: string[]; disallowed_paths: string[]; max_files_changed: number }
      changes: Array<{ kind: string; path: string; rationale: string }>
    }
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      const [f, p] = await Promise.all([
        readJson('/api/self-improve/findings'),
        readJson('/api/self-improve/proposals'),
      ])
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <p className="text-sm text-slate-300">
        Detects CI/policy/replay issues, drafts bounded proposals, and executes via governed PR
        flow.
      </p>
      ...
      {selected ? (
        <section className="rounded border p-4 text-sm space-y-2">
          ...
          <p>
            Finding: {selected.proposal.finding.type} ({selected.proposal.finding.severity})
          </p>
