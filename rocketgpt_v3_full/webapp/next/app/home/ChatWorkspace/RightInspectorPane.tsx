...
export default function RightInspectorPane() {
  ...
  return (
    <aside className="h-full border-l border-slate-900/80 bg-slate-950/80 p-3 flex flex-col overflow-y-auto space-y-3">
      <div className="text-xs font-semibold uppercase tracking-wide text-emerald-300">Design Mode</div>
        ...
        <>
          ...
          <InspectorCard title="Validation">
            ...
            <div className="text-[11px] text-slate-300">
              approval={validation.governanceSummary.anyRequiresApproval ? "required" : "optional"} | passport=
          ...
          <div className="space-y-2">
            {draftArtifact.nodes.map((node, index) => (
              <div key={node.node_id} className="rounded-lg border border-slate-800 bg-slate-950/50 p-3 space-y-2">
                ...
                <div className="flex items-center gap-2">
                  ...
                  {initParamErrors[node.node_id] ? (
                    <span className="text-[10px] text-rose-300">{initParamErrors[node.node_id]}</span>
                  ) : null}
                </div>
                ...
                <div className="space-y-1">
                  ...
                  {node.expected_outputs.map((output) => (
                    <div key={output.id} className="flex items-center gap-1">
                      ...
                      <input
                        value={output.label}
                        onChange={(event) => updateNodeOutputLabel(node.node_id, output.id, event.target.value)}
          ...
          <div className="pt-1 space-y-2">
            <button
              ...
              onClick={() =>
                router.push(`/workflows/builder?from=chat&draftId=${encodeURIComponent(draftArtifact.draft_id)}`)
              }
            ...
            <button
              type="button"
              onClick={() => downloadJson("chat-workflow-draft.json", { ...draftArtifact, validation })}
...
function InspectorCard({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="rounded-lg border border-slate-800 bg-slate-950/50 p-3 space-y-2">
      <div className="text-[11px] font-semibold text-slate-300 uppercase tracking-wide">{title}</div>
