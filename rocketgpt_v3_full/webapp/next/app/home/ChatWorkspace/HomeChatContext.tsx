...
function formatAssistantWorkflowMessage(
...
export function HomeChatProvider({ children }: { children: ReactNode }) {
  ...
  function setNodeExpectedBehavior(nodeId: string, expectedBehavior: string) {
    commitNodeEdits((nodes) =>
      nodes.map((node) =>
        node.node_id === nodeId ? { ...node, expected_behavior: expectedBehavior } : node,
      ),
    )
  }
  ...
  function applyInitParams(nodeId: string) {
    ...
    try {
      ...
      commitNodeEdits((nodes) =>
        nodes.map((node) => (node.node_id === nodeId ? { ...node, init_params: parsed } : node)),
      )
    } catch {
    ...
    commitNodeEdits((nodes) =>
      nodes.map((node) =>
        node.node_id !== nodeId
          ...
          : {
              ...node,
              expected_outputs: node.expected_outputs.filter((output) => output.id !== outputId),
            },
      ),
    )
  }
