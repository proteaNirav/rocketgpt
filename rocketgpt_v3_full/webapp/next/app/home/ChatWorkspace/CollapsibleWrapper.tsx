 ...
 */
export default function CollapsibleWrapper({ children, side }: CollapsibleWrapperProps) {
  ...
  const toggleLabel = side === 'left' ? (collapsed ? '›' : '‹') : collapsed ? '‹' : '›'
  ...
  return (
    ...
    >
      ...
      <div
        className={
          'flex-1 overflow-hidden ' + (collapsed ? 'opacity-0 pointer-events-none' : 'opacity-100')
        }
      >
        {children}
      </div>
