...
function isSafeProposalId(v: string) {
  ...
  return /^[A-Za-z0-9][A-Za-z0-9._-]{2,80}$/.test(v) && !v.includes("..") && !v.includes("/") && !v.includes("\\");
}
...
export async function POST(req: NextRequest) {
  ...
  if (!confirmed) {
    return NextResponse.json({ error: "Execution requires explicit confirmed=true." }, { status: 400 });
  }
