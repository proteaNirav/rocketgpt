...
const TESTER_URL = process.env.RGPT_TESTER_URL ?? 'http://localhost:3000/api/tester/run'
 ...
 */
export async function callTesterFromOrchestrator(payload: TesterRequest): Promise<TesterResponse> {
  ...
  if (!res.ok) {
    ...
    throw new Error(`Tester API responded with ${res.status}: ${res.statusText} ${text}`)
  }
