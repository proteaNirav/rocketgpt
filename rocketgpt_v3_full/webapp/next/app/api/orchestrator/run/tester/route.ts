...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  const decisionIdHeader =
    req.headers.get('x-rgpt-decision-id') ?? req.headers.get('X-RGPT-Decision-Id') ?? undefined
  try {
  ...
  return withOrchestratorHandler({ route: ROUTE, runId }, async () => {
    const route = ROUTE
    ...
    try {
      const res = await fetch(testerUrl, {
        ...
        headers: {
          ...
          ...(decisionIdHeader ? { 'x-rgpt-decision-id': decisionIdHeader } : {}),
      ...
      if (!res.ok) {
        const upstreamStatus = res.status >= 400 && res.status < 500 ? res.status : 502
        return NextResponse.json(
          {
            success: false,
