...
export async function runTesterEngine(
  ...
  try {
    logs.push('Calling /api/orchestrator/builder/execute-all to validate HTTP status...')

    const res = await fetch('http://localhost:3000/api/orchestrator/builder/execute-all', {
      method: 'POST',
      headers: {
        'content-type': 'application/json',
        ...(process.env.RGPT_INTERNAL_KEY
          ? { 'x-rgpt-internal': process.env.RGPT_INTERNAL_KEY }
          : {}),
      },
      body: JSON.stringify({}),
    })
    ...
    try {
      ...
      logs.push('Orchestrator/builder response body parsed as JSON (not stored in engine result).')
      logs.push(`Body.highlevel: ${bodyJson?.message ?? '[no message field]'}`)
    } catch {
    ...
    httpResult = 'error'
    httpMessage =
      'Error while calling /api/orchestrator/builder/execute-all: ' + (err?.message || String(err))
