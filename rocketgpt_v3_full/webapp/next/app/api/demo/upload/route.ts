...
export async function POST(req: NextRequest) {
  try {
    ...
    return NextResponse.json({
      ok: true,
      receivedFiles: count,
      message: 'Demo upload endpoint. Files are not persisted in this placeholder version.',
    })
  } catch (err: any) {
    console.error('Error in /api/demo/upload:', err)
    return NextResponse.json({ error: 'Internal error in /api/demo/upload' }, { status: 500 })
  }
}
