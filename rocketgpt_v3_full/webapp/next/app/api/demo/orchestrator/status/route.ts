...
export async function GET(_req: NextRequest) {
  ...
  try {
    ...
    const knowledgeIndexPath = path.join(dataRoot, 'self_study', 'knowledge_index.json')
    const ideaPoolPath = path.join(dataRoot, 'self_innovate', 'idea_pool.jsonl')
    const researchLogPath = path.join(dataRoot, 'self_research', 'research_log.jsonl')
    const domainsRoot = path.join(dataRoot, 'domains')

    const [knowledgeIndex, ideaCount, researchCount, domainCount] = await Promise.all([
