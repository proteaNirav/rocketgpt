...
export async function GET(req: NextRequest) {
  try {
    ...
    return NextResponse.json({ error: error instanceof Error ? error.message : "Failed to list CATS." }, { status: 400 });
  }
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!actor.isAdmin && !actor.userId) {
      return NextResponse.json({ error: "Owner user or admin token required for writes." }, { status: 401 });
    }
