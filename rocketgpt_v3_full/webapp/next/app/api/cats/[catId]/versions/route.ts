...
export async function GET(req: NextRequest, ctx: Ctx) {
  try {
    ...
    return NextResponse.json({ error: error instanceof Error ? error.message : "Failed to list versions." }, { status: 400 });
  }
}
...
export async function POST(req: NextRequest, ctx: Ctx) {
  try {
    ...
    if (!ALLOWED_PUBLISH_CAT_STATUSES.has(cat.status)) {
      return NextResponse.json({ error: "Version publish allowed only in draft/approved." }, { status: 400 });
    }

    const body = await req.json().catch(() => null);
    const version = String(body?.version || "").trim();
    const commandBundleRef = String(body?.commandBundleRef || "").trim();
    const manifestJson = body?.manifestJson && typeof body.manifestJson === "object" ? body.manifestJson : {};
    const rulebookJson = body?.rulebookJson && typeof body.rulebookJson === "object" ? body.rulebookJson : {};
