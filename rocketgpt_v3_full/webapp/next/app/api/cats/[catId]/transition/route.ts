...
const VALID_STATUS: CatsStatus[] = ["draft", "review", "approved", "active", "deprecated", "revoked"];
...
export async function POST(req: NextRequest, ctx: Ctx) {
  try {
    ...
    if (!canTransition(cat.status, targetStatus)) {
      return NextResponse.json({ error: `Invalid transition: ${cat.status} -> ${targetStatus}` }, { status: 400 });
    }
    ...
    return NextResponse.json({ error: error instanceof Error ? error.message : "Failed to transition CAT." }, { status: 400 });
  }
}
