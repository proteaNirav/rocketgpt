  ...
  return (
    <div className="p-6 space-y-4 text-gray-900 dark:text-gray-100">
      ...
      <p className="text-sm text-gray-600 dark:text-gray-400">
        This page will evolve into a full logs viewer with filters, search, and pagination. Data
        below currently comes from the <code>/api/logs</code> endpoint.
      </p>
      ...
      {!loading && !error && (
        ...
        >
          ...
          {logs.length === 0 ? (
            <p className="text-xs text-gray-600 dark:text-gray-400">No logs available yet.</p>
          ) : (
                ...
                <tbody>
                  {logs.map((log) => (
                    ...
                    >
                      ...
                      <td className="py-2 pr-4 text-gray-800 dark:text-gray-200">{log.endpoint}</td>
                      <td className="py-2 pr-4 text-gray-800 dark:text-gray-200">{log.model}</td>
          ...
          <p className="mt-3 text-[11px] text-gray-600 dark:text-gray-400">
            Later, this table will be backed by real request logs stored in Postgres/Supabase with
            filters for endpoint, model, status, and date ranges.
          </p>
        </div>
      )}
    </div>
