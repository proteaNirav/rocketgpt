...
exports.AdoptedRecallFoundation = void 0;
...
function metadataString(memory, key) {
    const value = memory.metadata?.[key];
    return typeof value === "string" && value.trim().length > 0 ? value : undefined;
}
function metadataStringArray(memory, key) {
    const value = memory.metadata?.[key];
...
function metadataNumber(memory, key) {
    const value = memory.metadata?.[key];
    return typeof value === "number" && Number.isFinite(value) ? value : undefined;
}
function hasTag(memory, key, value) {
    if (!value) {
        return memory.tags.some((tag) => tag.key === key);
    }
    return memory.tags.some((tag) => tag.key === key && tag.value === value);
}
function isMalformed(memory) {
    return (typeof memory.memoryId !== "string" ||
...
class AdoptedRecallFoundation {
    ...
    recall(input) {
        ...
        for (const memory of input.items) {
            ...
            const hasRiskySignal = signalTypes.includes("integrity_warning") ||
                signalTypes.includes("drift_detected") ||
                signalTypes.includes("verification_rejected");
            if (adoptionDecision === "suppressed" ||
        ...
        const scored = ranked.map((item) => {
            ...
            const quality = qualityRaw === "trusted" || qualityRaw === "warning" || qualityRaw === "degraded" ? qualityRaw : "unknown";
            ...
            const signalAdjustment = (signalTypes.includes("adoption_suppressed") ? -0.1 : 0) +
            ...
            const capabilityAdjustment = input.capabilityId && (hasTag(memory, "capability_id", input.capabilityId) || memory.content.includes(input.capabilityId))
                ? 0.08
                : 0;
            const routeAdjustment = input.routeType && (hasTag(memory, "route_type", input.routeType) || memory.content.includes(input.routeType))
                ? 0.05
                : 0;
...
exports.AdoptedRecallFoundation = AdoptedRecallFoundation;
