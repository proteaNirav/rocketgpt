    ...
    constructor(metrics, options) {
        ...
        this.signalFactory = options?.signalRouter
            ? new signal_factory_1.SignalFactory({
                ...
                defaultPriority: signal_priority_1.CognitiveSignalPriority.NORMAL,
            })
            : undefined;
    }
    async plan(event, contextText, recall) {
        if (this.signalRouter && this.signalFactory) {
            void this.signalRouter.emit(this.signalFactory.create({
                signalType: signal_types_1.CognitiveSignalType.REASONING_REQUIRED,
