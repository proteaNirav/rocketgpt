...
(0, node_test_1.test)("high urgency plus high risk produces high attention", () => {
    const engine = new cognitive_attention_engine_1.CognitiveAttentionEngine({ now: () => BASE_TS });
...
(0, node_test_1.test)("near deadline increases attention score", () => {
    const engine = new cognitive_attention_engine_1.CognitiveAttentionEngine({ now: () => BASE_TS });
    const withoutDeadline = engine.evaluate(createInput({ id: "without-deadline", urgency: 0.5, risk: 0.4 }));
    const nearDeadline = engine.evaluate(createInput({
        id: "near-deadline",
        urgency: 0.5,
        risk: 0.4,
...
(0, node_test_1.test)("low-value item remains in low band", () => {
    const engine = new cognitive_attention_engine_1.CognitiveAttentionEngine({ now: () => BASE_TS });
    const result = engine.evaluate(createInput({
        id: "low",
        urgency: 0.05,
...
(0, node_test_1.test)("rank sorts attention results by descending score", () => {
    const engine = new cognitive_attention_engine_1.CognitiveAttentionEngine({ now: () => BASE_TS });
...
(0, node_test_1.test)("rank ordering is deterministic when scores are equal", () => {
    const engine = new cognitive_attention_engine_1.CognitiveAttentionEngine({ now: () => BASE_TS });
    ...
    assert.deepEqual(tied.map((item) => item.id), ["a", "m", "z"]);
});
(0, node_test_1.test)("top returns stable subset and evaluate clamps NaN/Infinity safely", () => {
    const engine = new cognitive_attention_engine_1.CognitiveAttentionEngine({ now: () => BASE_TS });
    ...
    assert.deepEqual(top.map((item) => item.id), ["y", "z"]);
