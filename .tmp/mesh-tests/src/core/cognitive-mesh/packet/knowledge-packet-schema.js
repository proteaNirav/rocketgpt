...
exports.validateKnowledgePacketSchema = validateKnowledgePacketSchema;
...
function validateKnowledgePacketSchema(packet) {
    const issues = [];
    ...
    if (candidate.protocolVersion !== knowledge_packet_types_1.KNOWLEDGE_PACKET_PROTOCOL_VERSION) {
        issues.push({
            path: 'protocolVersion',
            message: `Expected protocolVersion ${knowledge_packet_types_1.KNOWLEDGE_PACKET_PROTOCOL_VERSION}.`,
        });
    }
    ...
    if (!isPlainObject(candidate.trust)) {
        ...
        if (candidate.trust.trustLevel !== 'internal' &&
    ...
    if (!isPlainObject(candidate.routing)) {
        ...
        if (candidate.routing.strategy !== 'direct' &&
        ...
        if (candidate.routing.deliveryMode !== 'single' &&
        ...
        if (candidate.routing.preferredTargets !== undefined &&
        ...
        if (Array.isArray(candidate.routing.preferredTargets) &&
        ...
        if (candidate.routing.capabilityTags !== undefined &&
    ...
    else if (candidate.tags.some((value) => typeof value !== 'string' || value.trim().length === 0)) {
    ...
    if (!isPlainObject(candidate.integrity)) {
        ...
        if (candidate.integrity.algorithm !== knowledge_packet_types_1.KNOWLEDGE_PACKET_INTEGRITY_ALGORITHM) {
            issues.push({
                path: 'integrity.algorithm',
                message: `Expected integrity algorithm ${knowledge_packet_types_1.KNOWLEDGE_PACKET_INTEGRITY_ALGORITHM}.`,
            });
        }
        if (typeof candidate.integrity.digest !== 'string' ||
        ...
        if (typeof candidate.integrity.canonicalPayloadDigest !== 'string' ||
