...
exports.verifyCapabilityResult = verifyCapabilityResult;
...
const POLICY_REJECTED_STATUSES = new Set(["denied", "blocked"]);
function normalizeReasonCodes(codes) {
    return [...new Set(codes)].sort();
}
function mapFailureClassByStatus(status) {
    switch (status) {
        case "success":
            return "none";
...
function isValidLifecycleStage(value) {
    return (value === "capability_resolution" ||
...
function verifyCapabilityResult(input) {
    const reasonCodes = [];
    const warnings = [];
    ...
    if ((input.runtimeGuardOutcome === "deny" || input.runtimeGuardOutcome === "safe_mode_redirect") &&
        (status === "success" || status === "degraded_success")) {
        reasonCodes.push("RESULT_STATUS_POLICY_REJECTED");
    }
    if ((input.dispatchGuardOutcome === "deny" || input.dispatchGuardOutcome === "safe_mode_redirect") &&
        (status === "success" || status === "degraded_success")) {
        reasonCodes.push("RESULT_STATUS_POLICY_REJECTED");
    }
    ...
    if (normalizedReasonCodes.length > 0) {
        ...
        const hasInvalidity = normalizedReasonCodes.includes("RESULT_REQUEST_ID_MISMATCH") ||
