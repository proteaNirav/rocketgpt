...
def _inspector_stage(ctx: ReplayContext, collector_report: Dict[str, Any]) -> Dict[str, Any]:
    # Best-effort: if inputs missing, mark as UNKNOWN rather than hard-fail
    def tri_from_missing(required_key: str) -> TriState:
        return "UNKNOWN" if required_key in collector_report.get("missing_inputs", []) else "UNKNOWN"

    checks: Dict[str, TriState] = {
        "ledger_hash_chain_valid": tri_from_missing("execution_ledger_path"),
        "artifact_hashes_valid": tri_from_missing("artifacts_manifest_path"),
        "schema_versions_compatible": tri_from_missing("execution_ledger_path"),
    }

    errors: List[str] = []

    # Attempt validations only if corresponding file exists
    ...
    # For now, inspector PASS means "report produced", not integrity success
    # (Commissioner will strictly gate based on must_have)
    return {
        "stage": "inspector",
        "checks": checks,
        "status": "PASS",
...
def _commissioner_stage(ctx: ReplayContext, inspector_report: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # NOTE: Tenant/org permission checks are not implemented yet -> UNKNOWN
    # This is intentional: contract says strict from stage 3; UNKNOWN will DENY by default.
...
def build_context(contract: Dict[str, Any]) -> ReplayContext:
    ...
    evidence_dir = outputs.get("evidence_dir", "").strip()
    stage_reports_dir = outputs.get("stage_reports_dir", "").strip()
...
def main() -> int:
    ...
    ap.add_argument(
        "--contract",
        default="apps/core-api/replay/replay_contract.json",
        help="Path to replay_contract.json",
    )
    args = ap.parse_args()
